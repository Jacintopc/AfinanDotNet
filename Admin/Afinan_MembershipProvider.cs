...
// 
// http://msdn.microsoft.com/en-us/library/ms366730(v=VS.85).aspx?appId=Dev10IDEF1&l=EN-US&k=k(ENCRYPTPASSWORD)%3bk(TargetFrameworkMoniker-%22.NETFRAMEWORK&k=VERSION%3dV2.0%22)%3bk(DevLang-CSHARP)&rd=true
// 

namespace AfinanDotNet2._0.Admin
{
    public class Afinan_MembershipProvider : MembershipProvider    // public sealed class OleDbMembershipProvider : MembershipProvider
    {
        ...
        private string eventSource = "Afinan_MembershipProvider";
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (name == null || name.Length == 0)
                name = "Afinan_MembershipProvider";

            if (String.IsNullOrEmpty(config["description"]))
            {
                config.Remove("description");
                config.Add("description", "Afinan Membership provider");
            }
        ...
        public override bool ChangePassword(string username, string oldPwd, string newPwd)
        {
            ...
            // UPDATE [Users] SET Password = 	"qj4rYGc+Iy63KriPe2hqOma1/xY=" , LastPasswordChangedDate = "02/07/2010 12:19:08"
            // WHERE Username = "AFINAN" AND ApplicationName = "AfinanDotNet2.0"	
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add("@ApplicationName", OleDbType.VarChar, 255).Value = ApplicationName;  //jacinto creo que falta esto.
                ...
                if (CheckPassword(password, pwd))
                {
                    if (isApproved)
                    {
                        ...
                        OleDbCommand updateCmd = new OleDbCommand("UPDATE [" + tableName + "] SET LastLoginDate = ?, " +
                                                                  " IsLockedOut = False , FailedPasswordAttemptCount = 0 " +
                                                                  " WHERE Username = ? AND ApplicationName = ?", conn);
        ...
        public string EncodePassword(string password)   // antes era un método privado
        {
